...
export async function generateIncentiveProgram(
  userSelections: Decision[]
): Promise<AIResponse> {
  try {
    // Step 1: Generate query embedding
    const queryEmbedding = await generateQueryEmbedding(
      userSelections.map((d) => d.value)
    );
    ...
    const systemPrompt =
      "You are an expert assistant helping to design an incentive program.";
...
function getDefaultAIResponse(): AIResponse {
  return {
    ...
    nextSteps: ["Unable to generate next steps."],
    metrics: {
      design: "N/A",
      implementation: "N/A",
      monitoring: "N/A",
...
async function main() {
  const userSelections: Decision[] = [
    {
      type: "Goal",
      value: "Increase customer engagement",
      question: "What is your main goal?",
      answer: "Increase customer engagement",
    },
    {
      type: "Audience",
      value: "New users",
    ...
    },
    {
      ...
      question: "What type of incentive are you offering?",
